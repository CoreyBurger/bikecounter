...
for index,row in counterList.iterrows():
    ...
    totalCountString = "Since " + row[['FirstDate']][0].strftime('%A, %b %d, %Y') + " there have been " + locale.format_string("%d", totalRides, grouping=True) + " rides past the counter"
    ...
    with open (workingDir + '\\counterVisual-' + str(row[['CounterID']][0]) + '.html',"w") as counterPage:
